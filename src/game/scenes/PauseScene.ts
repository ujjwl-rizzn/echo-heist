...
export class PauseScene extends Phaser.Scene {
  ...
  create(data: { levelId?:string } | undefined): void {
    ...
    uiManager.showPause({
      ...
      onRetry: () => {
        audioManager.playUi(); uiManager.clearScreen();
        this.scene.stop(SCENE_KEYS.GAME); this.scene.stop();
        this.scene.start(SCENE_KEYS.GAME, { levelId });
      },
      onSettings: () => {
        audioManager.playUi(); uiManager.clearScreen(); this.scene.stop();
        this.scene.launch(SCENE_KEYS.SETTINGS, { returnScene:SCENE_KEYS.PAUSE, levelId });
      },
      onMenu: () => {
        audioManager.playUi(); audioManager.setMusicMode("menu");
        uiManager.clearScreen(); this.scene.stop(SCENE_KEYS.GAME); this.scene.stop();
